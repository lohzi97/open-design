...
Use `127.0.0.1`, not `localhost`. The container binds the daemon to `0.0.0.0`,
and that binding reserves the alternate loopback origin (`localhost`) for
embedded powered previews; browsing via `http://localhost:7456` makes every
`/api/*` request fail with `403 Powered preview origin cannot access this API
route`.
...
- **Docker: every `/api/*` request returns 403 `Powered preview origin cannot access this API route`** — the app was opened via `http://localhost:7456`. Docker mode binds the daemon to `0.0.0.0`, which reserves `localhost` for powered previews. Open `http://127.0.0.1:7456` instead. If the browser then shows its native sign-in dialog, enter `open-design` as the username and the `OD_API_TOKEN` value from `deploy/.env` as the password.
